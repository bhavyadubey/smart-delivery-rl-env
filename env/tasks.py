...
def task_easy():
    ...
    return min(1.0, total_reward / 50)
...
def task_medium():
    ...
    for _ in range(20):
        best = max(obs.deliveries, key=lambda d: d.priority if not d.done else -1)
    ...
    return min(1.0, total_reward / 100)
...
def task_hard():
    ...
    return min(1.0, total_reward / 150)
